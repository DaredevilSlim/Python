...
# число можно не всегда. Теперь они хотят понять, какое минимальное и какое максимальное количество цапель могло быть в
# вольере.
...
# Входные данные - Входной файл INPUT.TXT содержит два целых числа a и b, разделенных ровно одним пробелом (1 ≤ a ≤ 109,
# 1 ≤ b ≤ 109).
# Выходные данные - В выходной файл OUTPUT.TXT выведите два целых числа, разделенных пробелом — минимальное и
# максимальное число цапель, которое могло быть в вольере. Гарантируется, что хотя бы одно количество цапель
# соответствует условию задачи.
a, b = sorted(map(int, input().split()))
print(b // 2 + b % 2, a)
# Или
a, b = sorted(map(int, input().split()))
print(b // 2 if b % 2 == 0 else b // 2 + 1, a)
