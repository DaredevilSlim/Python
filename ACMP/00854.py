...
# такне больше желаемой, то он выключается.
...
# Входные данные - Первая строка входного файла INPUT.TXT содержит два целых числа troom, и tcond, разделенных ровно
# одним пробелом (–50 ≤ troom ≤ 50, –50 ≤ tcond ≤ 50).
# Вторая строка содержит одно слово, записанное строчными буквами английского алфавита — режим работы кондиционера, как
# указано выше.
# Выходные данные - Выходной файл OUTPUT.TXT должен содержать одно целое число — температуру, которая установится в
# комнате через час.
a, b = map(int, input().split())
c = input()
print(max(a, b) if c == 'heat' else a if c == 'fan' else b if c == 'auto' else min(a, b))
# Или
a, b = map(int, input().split())
c = input()
if c == 'heat':
