...
import random  # Подключаем модуля random
...
# def <Имя функции>([<Строго позиционные параметры>, /,] <Двоякие параметры>[, *<Список позиционных параметров>]
#                   [, <Строго именованные параметры>][, **<Словарь именованных параметров>])
...
# Пример использования декоратора показан в test_00068.py.
# Результат выполнения этого примера:
# Вызвана функция func()
# x = 10
# Приведенный пример эквивалентен коду, показанному в test_00069.py.
# У определения изменяемой функции можно указать сразу несколько декораторов. Для примера укажем у функции func() два
# декоратора: deco1() и deco2() test_00070.py.
...
# Здесь сначала будет вызвана функция deco2(), а затем функция deco1(). Результат выполнения будет присвоен
# идентификатору func.
# В качестве еще одного примера использования декораторов рассмотрим выполнение функции только при правильно введенном
# пароле test_00071.py.
# Здесь после символа @ указана не ссылка на функцию, а выражение, возвращающее декоратор. Иными словами, декоратором
# здесь служит не функция test_passw(), а результат ее выполнения (функция deco()). Если введенный пароль является
# правильным, то выполнится функция func(), в противном случае будет выведена надпись "Доступ закрыт", которую вернет
# анонимная функция.


# 11.5. Рекурсия
print('11.5. Рекурсия')
# Рекурсия - это вызов функцией самой себя. Рекурсию удобно использовать для перебора объекта с неизвестной структурой
# или для выполнения неопределенного количества операций. Для примера рассмотрим вычисление факториала test_00072.py.
# Впрочем, для вычисления факториала лучше воспользоваться функцией factorial() из модуля math (см. разд. 5.3).
# Количество вызовов функции самой себя (проходов рекурсии) ограничено. Узнать его можно, вызвав функцию
# getrecursionlimit() из модуля sys:
...
print('11.6. Вложенные функции')
# Вложенная функция - это функция, определенная в теле другой функции (родительской).
# Вложенная функция может создавать свои собственные локальные переменные и имеет доступ к локальным переменным,
# созданным в родительской функции. Рассмотрим пример из test_00073.py.
# Здесь мы определили функцию func1(), принимающую один параметр, а внутри нее - вложенную функцию func2(). Результатом
# выполнения функции func1() станет ссылка на эту вложенную функцию. Внутри функции func2() мы производим вывод значения
# переменной х, созданной в функции func1().
# Следует учитывать, что в момент определения функции сохраняются ссылки на переменные, а не их значения. Например, если
# после определения функции func2() произвести изменение переменной х, то будет использоваться это новое значение
# test_00074.py.
# Обратите внимание на результат выполнения - в обоих случаях мы получили значение 30. Если при определении вложенной
# функции необходимо сохранить именно значение переменной, следует указать его как значение по умолчанию test_00075.py.
# Теперь попробуем из вложенной функции func2() изменить значение переменной х, созданной внутри функции func1(). Если
# внутри функции func2() присвоить значение переменной х, будет создана новая локальная переменная с таким же именем.
# Если внутри функции func2() объявить переменную как глобальную и присвоить ей значение, то изменится значение
# глобальной переменной, а не значение переменной х из функции func1().
# Таким образом, ни один из изученных ранее способов не позволяет из вложенной функции изменить значение локальной
# переменной из родительской функции. Решить эту проблему можно, объявив необходимые переменные с помощью языковой
# конструкции формата:
# nonlocal <Имена переменных через запятую>
# Пример показан в test_00076.py.
# При использовании языковой конструкции nonlocal следует помнить, что переменная обязательно должна существовать внутри
# функции-родителя. В противном случае будет выведено сообщение об ошибке.


# 11.7. Аннотации функций
print('11.7. Аннотации функций')
# Аннотация - это описание назначения параметров функции, их типов и типа возвращаемого результата, которое записывается
# в определении функции. Аннотации имеют следующий формат:
# def <Имя функции>([<Параметр 1>[: <Описание параметра>][= <Значение по умолчанию>][, ... , <Параметр N>
# [: <Описание параметра>][= <Значение по умолчанию>]]]) -> <Описание возвращаемого результата>: <Тело функции>
# В качестве описаний параметров и возвращаемого результата обычно указываются строки (хотя можно задать любое выражение
# языка Python, которое будет выполнено при создании функции). Пример:
...
# У параметра 'a' задано описание 'Выводимое значение 1', у параметра 'b' - 'Выводимое значение 2' (число 3 - это
# значение параметра b по умолчанию). После закрывающей круглой скобки указан тип возвращаемого функцией результата:
# None (т. е. результата функция не возвращает).
# Аннотация функции сохраняется в виде словаря в атрибуте __annotations__ ее объекта.
# Выведем значение этого атрибута:
