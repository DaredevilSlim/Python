...
#     ^^^^^^^^^^^^^^^^^^
# ValueError
# Четвертый формат инструкции повторно генерирует последнее возникшее исключение и обычно применяется в коде, следующем
# за конструкцией except. Пример использования этого формата показан в test_00123.py.


# 14.5. Пользовательские исключения
print('14.5. Пользовательские исключения')
# Пользовательское исключение - это исключение, созданное программистом. В качестве пользовательского исключения всегда
# выступает класс, производный от класса Exception.
# Пример определения и использования пользовательского исключения показан в test_00124.py.
# Класс Exception поддерживает все необходимые методы для вывода сообщения об ошибке.
# Поэтому в большинстве случаев для создания пользовательского исключения достаточно определить пустой класс,
# производный от него, показан в test_00125.py.


# 14.6. Проверочная инструкция
print('14.6. Проверочная инструкция')
# Проверочная инструкция позволяет удостовериться, что заданное условие выполняется.
# Если же это не так, инструкция сгенерирует исключение AssertionError.
...
# Если при запуске программы используется командный ключ -o, то переменная __debug__ будет хранить значение False. Таким
# образом, если запустить программу без ключа -o, проверочные инструкции выполняться не будут.
# Пример использования инструкции assert представлен в test_00126.py.
