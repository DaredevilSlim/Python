...
def target_indices(nums: list[int], target: int) -> list[int]:
    nums = sorted(nums)
    return [i for i in range(len(nums)) if target == nums[i]]


print(target_indices([1, 2, 5, 2, 3], 2))  # [1, 2]
print(target_indices([1, 2, 5, 2, 3], 3))  # [3]
print(target_indices([1, 2, 5, 2, 3], 5))  # [4]
