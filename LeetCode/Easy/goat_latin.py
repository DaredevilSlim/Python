...
def to_goat_latin(sentence: str) -> str:
    ...
    for i in range(len(sentence)):
        if sentence[i][0].lower() not in ['a', 'e', 'i', 'o', 'u']:
            sentence[i] = sentence[i][1:] + sentence[i][0]
        new += ' ' + sentence[i] + 'ma' + 'a' * (i + 1)
    return new[1:]



print(to_goat_latin('I speak Goat Latin'))  # 'Imaa peaksmaaa oatGmaaaa atinLmaaaaa'
print(to_goat_latin('The quick brown fox jumped over the lazy dog'))  # 'heTmaa uickqmaaa rownbmaaaa oxfmaaaaa
