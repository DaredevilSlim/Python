...
# --рrоху=[<Имя пользователя>:<Пароль пользователя>@]<Интернет-адрес>:<Номер порта>
# Пример:
...
# консоли ее имя.
# Если Python был установлен в каталог c:\Pгogram Files (х86) или c:\Program Files, дополнительные библиотеки будут
# устанавливаться по пути с:\Usеrs\<Имя учетной записи>\АррDаtа\Roaming\Python\Python<Hoмep вepcии>\site-packages, а
# программы, входящие в состав библиотек, - по пути
# с:\Usеrs\<Имя учетной зanиcи>\AppData\Roaming\Python\Python<Hoмep вepcии>\Scripts. Путь к каталогу программ не
...
# запустится сервер документов pydoc. Это написанный на самом Python веб-сервер, выводящий в веб-браузере документацшо
...
# FILE
#     /media/daredevilslim/Samsung/Programming/Python/Scripts/Python3_Developing_Applications/Examples/test_00008.py
...
# 1.1О. Компиляция Руthоn-файлов
# При запуске любой Руthоn-программы, сохраненной в файле, интерпретатор предварительно выполняет ее компиляцию -
...
# Откомпилированные модули сохранятся в каталоге _pycache_, который создается в каталоге с исходными Руthоn-модулями, в
...
# простым щелчком мышью. А при распространении Руthоn-программы можно передать конечным пользователям только
