...
# числа имеют самый простой тип, далее идут веще­ственные числа и самый сложный тип - комплексные числа. Таким образом,
# если в опера­ции участвуют целое число и вещественное, то целое число будет автоматически преобразовано в веществен
# ное число, а затем произведена операция над вещественными числами. Результатом этой операции будет вещественное число.
# Создать объект целочисленного типа можно обычным способом:
x = 0
y = 10
z = -80
print(x, y, z)
# Также можно указать число в двоичной, восьмеричной или шестнадцатеричной фор­ме. Такие числа будут автоматически
...
# int([<Объект>[, <Система счисления>]]) - преобразует объект в целое число. Во вто­ром параметре можно указать систему
...
print(float())
# Ьin(<Число>) - преобразует десятичное число в двоичное. Возвращает строковое представление числа.
print('Ьin(<Число>) - преобразует десятичное число в двоичное:')
...
# округленное до ближайшего меньшего целого, а для чи­сел с дробной частью больше 0.5 возвращает число, округленное до
# ближайшего боль­шего целого. Если дробная часть равна 0.5, то округление производится до ближайшего четного числа.
...
# Если указан третий параметр, то возвращается остаток от деления полученного резуль­тата на значение этого параметра:
