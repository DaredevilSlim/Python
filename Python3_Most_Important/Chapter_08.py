...
# списком, кортежем, словарем и т.д. Создать вложен­ный список можно, например, так:
...
# не содержит опера­тора присваивания, - на каждой итерации цикла будет генерироваться новый элемент, которому неявным
...
print(arr)                      # Результат выполнения: [20, 40, 60]
# Если выражение разместить внутри не квадратных, а круглых скобок, то будет возвращать­ся не список, а итератор. Такие
...
# существуют во всех последовательностях на одинаковом сме­щении:
...
# Если в первом параметре вместо названия функции указать значение None, то каждый эле­мент последовательности будет
...
# В первом параметре можно указать ссылку на функцию. В эту функцию в качестве пара­метра будет передаваться текущий
...
# В параметр <Функция> в качестве параметров передаются два элемента: первый эле­мент будет содержать результат
...
# операцией присваивания значения срезу. Добавим не­сколько элементов в начало списка:
arr = [1, 2, 3]
arr[:0] = [-2, -1, 0]
print(arr)
# рор([<Индекс>]) - удаляет элемент, расположенный по указанному индексу, и воз­вращает его. Если индекс не указан, то
...
# исключение ValueError. Метод изменяет текущий спи­сок и ничего не возвращает.
arr = [1, 2, 3, 1, 1]
...
# множество обратно преобразовать в список. Обратите внима­ние на то, что список должен содержать только неизменяемые
...
# Как вы уже знаете, выполнить проверку на вхождение элемента в список позволяет опера­тор in: если элемент входит в
# список, то возвращается значение True, в противном слу­чае - False. Аналогичный оператор not in выполняет проверку на
...
# Тем не менее, оба этих оператора не дают никакой информации о местонахождении эле­мента внутри списка. Чтобы узнать
...
# возбуждается исключение ValueError. Если второй и третий пара­метры не указаны, то поиск будет производиться с начала
# и до конца списка.
arr = [1, 2, 1, 2, 1]
...
print(arr)
# Если необходимо изменить порядок следования и получить новый список, то следует вос­пользоваться функцией
# reversed(<Последовательность>). Функция возвращает итератор, который можно преобразовать в список с помощью функции
# list() или генератора спи­сков:
...
# choice(<Последовательность>) - возвращает случайный: элемент из любой последова­тельности (строки, списка, кортежа):
...
# слу­чаях необходимо получить отсортированный список, а текущий список оставить без изменений. Для этого следует
...
# <Шаг> не указан, то используется значе­ние 1. Заполнение списка числами от о до 10:
print(list(range(11)))
...
# Преобразовать список в строку позволяет метод join(). Элементы добавляются через ука­занный разделитель. Формат
# метода:
...
# Если параметр не указан, то создается пустой кор­теж.
...
# Четвертая строка в исходном примере также доказывает, что не скобки формируют кор­теж, а запятые. Помните, что любое
# выражение в языке Python можно заключить в круг­лые скобки, а чтобы получить кортеж, необходимо указать запятые.
...
# изменить. Объявить множество можно с помощью функ­ции frozenset():
f = frozenset()
print(f)
...
# памяти. Однако в диапазон мо­гут входить лишь последовательно стоящие друг за другом числа - сформировать диапазон на
...
# достигнут конец последовательности, перебор начнет­ся сначала, и так бесконечно.
n = 1
...
# на одинаковом смещении. Если последователь­ности имеют разное количество элементов, то вместо отсутствующего элемента
...
print(list(itertools.chain('abc', ['defg', 'hij'])))
# chain.from_iterable(<Последовательность>) - аналогична функции chain(), но прини­мает одну последовательность, каждый
...
# последовательности. Если второй параметр не ука­зан, то возвращается кортеж из двух итераторов.
arr = [1, 2, 3]
