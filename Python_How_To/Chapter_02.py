...
# Текстовая информация — самая важная форма данных практически в любом приложении. Текстовые данные, как и числовые,
# можно сохранять в текстовых файлах, а чтение из таких файлов требует обработки строк. На веб-сайте интернет-магазина,
# например, текст используется для вывода описания товаров. Машинное обучение сейчас в моде, и вы, возможно, уже слышали
# об одной из его специализаций — обработке естественных языков, направленной на извлечение информации из текстов. Из-за
# повсеместного использования строк обработка текстов становится неизбежным шагом подготовки данных в таких сценариях.
# В контексте нашего таск-менеджера атрибуты задачи нужно преобразовать в текстовые данные, чтобы они могли отображаться
# интерфейсной частью веб-приложения. При получении данных от интерфейсной части приложения необходимо преобразовать
# эти строки к правильному типу (например, к целому) для дальнейшей обработки. Необходимость правильной обработки и
# форматирования строк возникает во множестве реальных ситуаций. В этой главе будут рассмотрены некоторые
...
# В языке Python предусмотрены разные способы форматирования текстовых строк. Один из популярных способов основан на
# использовании f-строк, позволяющих встраивать выражения в строковый литерал. Можно использовать и другие методы
# форматирования, но решения с f-строками лучше читаются, следовательно, f-строки становятся предпочтительным вариантом
# при подготовке строк для вывода.
# ОБРАТИТЕ ВНИМАНИЕ:
# F-строки впервые появились в Python 3.6. Префиксом f-строки может быть как символ f, так и символ F (от слова
# formatted - отформатированный). Строковый литерал представляет собой последовательность символов, заключенную в
# одинарные или двойные кавычки.
# При использовании строк для вывода часто приходится иметь дело с не строковыми данными, например целыми числами и
# числами с плавающей точкой. Допустим, таск-менеджер требует создания строкового вывода с использованием существующих
# переменных:
...
# В этом разделе вы научитесь использовать f-строки для интерполяции не строковых данных и представления строк в нужном
# формате. Вы увидите, что при форматировании строк с использованием существующих строк и других разновидностей
# переменных f-строки являются лучшим решением с точки зрения удобочитаемости.
