#!/usr/bin/env python3
# -*- coding: utf-8 -*-

# Эта программа вычисляет выплату продавцу в компании 'Создай свою музыку'.
def main():
    # Получить сумму продаж.
    sales = get_sales()
    # Получить сумму авансовой оплаты.
    advanced_pay = get_advanced_pay()
    # Определить ставку комиссионных.
    comm_rate = determine_comm_rate(sales)
    # Рассчитать оплату.
    pay = sales * comm_rate - advanced_pay
    # Показать сумму выплаты.
    print(f'Выплата составляет ${pay:,.2f}.')
    # Определить, является ли выплата отрицательной.
    if pay < 0:
        print('Продавец должен возместить разницу компании.')


# Функция get_sales получает от пользователя месячные продажи продавца и возвращает это значение.
def get_sales():
    # Получить сумму месячных продаж.
    monthly_sales = float(input('Введите сумму месячных продаж: '))
    # Вернуть введенную сумму.
    return monthly_sales


# Функция get_advanced_pay получает сумму авансовой выплаты конкретному продавцу и возвращает эту сумму.
def get_advanced_pay():
    # Получить сумму авансовой выплаты.
    print('Введите сумму авансовой выплаты либо введите 0, если такой выплаты не было.')
    advanced = float(input('Авансовая выплата: '))
    # Вернуть введенную сумму.
    return advanced


# Функция determine_comm_rate принимает сумму продаж в качестве аргумента и возвращает подходящую ставку комиссионных.
def determine_comm_rate(sales):
    # Определить ставку комиссионных.
    if sales < 10000.00:
        rate = 0.10
    elif 10000 <= sales <= 14999.99:
        rate = 0.12
    elif 15000 <= sales <= 17999.99:
        rate = 0.14
    elif 18000 <= sales <= 21999.99:
        rate = 0.16
    else:
        rate = 0.18
    # Вернуть ставку комиссионных.
    return rate
