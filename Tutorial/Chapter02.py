...
# The Python interpreter is usually installed as /usr/local/bin/python3.10 on those machines where it is available;
...
# On Windows machines where you have installed Python from the Microsoft Store, the python3.10 command will be
# available. If you have the py.exe launcher installed, you can use the py command. See Excursus: Setting environment
# variables for other ways to launch Python.
# Typing an end-of-file character (Control-D on Unix, Control-Z on Windows) at the primary prompt causes the interpreter
...
# systems that support the GNU Readline library. Perhaps the quickest check to see whether command line editing is
# supported is typing Control-P to the first Python prompt you get. If it beeps, you have command line editing;
# see Appendix Interactive Input Editing and History Substitution for an introduction to the keys. If nothing appears to
# happen, or if ^P is echoed, command line editing isn’t available; you’ll only be able to use backspace to remove
...
# command or -m module are not consumed by the Python interpreter’s option processing but left in sys.argv for the
...
# world can be used simultaneously in string literals, identifiers and comments — although the standard library only
# uses ASCII characters for identifiers, a convention that any portable code should follow. To display all these
# characters properly, your editor must recognize that the file is UTF-8, and it must use a font that supports all the
# characters in the file.
# To declare an encoding other than the default one, a special comment line should be added as the first line of the
